...
import 'package:alice/helper/alice_qa_export_helper.dart';
...
class AliceCore {
  ...
  /// Enable headless JSON export of all completed calls after each response.
  /// The file is written to app-specific external storage and can be pulled
  /// via ADB: adb pull <externalDir>/alice_qa_export.json
  void enableQAExport() {
    _qaExportEnabled = true;
  }
  ...
  void addResponse(AliceHttpResponse response, int requestId) {
    ...
    callsSubject.add([...callsSubject.value]);
    if (_qaExportEnabled) {
      unawaited(AliceQAExportHelper.export(callsSubject.value));
    }
  }
