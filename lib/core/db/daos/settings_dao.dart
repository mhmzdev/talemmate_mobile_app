...
class SettingsDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<AppearancePreferencesRow?> appearanceForUser(String userId) => (select(
    appearancePrefsTable,
  )..where((s) => s.userId.equals(userId))).getSingleOrNull();

  Future<LanguagePreferencesRow?> languageForUser(String userId) => (select(
    languagePrefsTable,
  )..where((s) => s.userId.equals(userId))).getSingleOrNull();

  Future<PrivacySettingsRow?> privacyForUser(String userId) => (select(
    privacySettingsTable,
  )..where((s) => s.userId.equals(userId))).getSingleOrNull();
